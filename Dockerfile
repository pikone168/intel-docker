...
RUN echo 'pikone' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/pikone.sh

RUN echo 'cd ' >>/pikone.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/pikone.sh

RUN echo 'cd /noVNC-1.2.0' >>/pikone.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/pikone.sh

RUN chmod 755 /pikone.sh
